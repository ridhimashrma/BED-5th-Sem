class OrderBook {
    ...
    placeOrder(price, quantity, type, side, userName,symbol) {
        ...
        if (newOrder.type == "LIMIT") {
            let [order, trade] = ob._LimitMatch(newOrder,trades);
            if(trade) {
                ob.trades = [...ob.trades,...trade];
            }
            ...
            return {book: ob.getBookSnapshot(), trades: ob.getLatestTrade(),order};

        } else {
            let [order, trade] = ob._MarketMatch(newOrder, trades);
            if(trade) {
                ob.trades = [...ob.trades,...trade];
            }
            if(order.remainingQty > 0) {
...
OrderBook.orderBookManager.set("BTC_USD", BTCUSDOrderBook);
...
BTCUSDOrderBook.placeOrder(null,10,"MARKET","BUY","Ridhima");
console.log(BTCUSDOrderBook.getPrice())
// console.log(BTCUSDOrderBook)
